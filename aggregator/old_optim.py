...
def optim_frankwolfe(params, agents_list, suffix: str, lambda_start=np.zeros(48)):
    ...
    lists_for_graphics = Graphic_lists()
    list_of_iterations = []
    ...
    while (num_it <= params.max_iter_FrankWolfe):
        ...
        current_iteration.lambdas = derivarite_f0(params, profile_aggreg,agent_averaged_profile_dict, agents_list)
        ...
        #         profiles_dict_per_it[num_it][key] = profiles_dict_per_it[num_it-1][key]  ## keep in memory successive agents profiles through iterations

        #     for key in costs_dict_per_it[num_it-1]:
        #         costs_dict_per_it[num_it][key] = costs_dict_per_it[num_it-1][key]  ## keep in memory successive agents costs through iterations

        ##### A REFACTORISER : 
        if num_it<=1:
            num_try=1
            lists_for_graphics.num_try_list.append(num_try)
        ...
        if parallel_run:
            N = np.sum(flags != 0)
            work = [(agent, current_iteration.lambdas[agent.name], params.date) for k, agent in enumerate(agents_list) if flags[k]]
            ...
            try:
                if flags[k]:
                    if not parallel_run:
                        ...
                        agent.tcl_update_load(current_iteration.lambdas[agent.name], path = path)
                    else:
        ...
        if num_it>0 and params.fully_corrective:
            ...
            lists_for_graphics.step_it_list.append(step_it)
            ...
            lists_for_graphics.cout_min_liste.append(cout_min)
        ...
        print(f'Cost of current strat : {avg_cost}')
        
        lists_for_graphics.facture_list.append((sum([costs_dict_per_it[num_it][agent.name] for agent in agents_list])+costs_dict_per_it[num_it]["market"])/len(agents_list))
        ...
        lists_for_graphics.upper_bound_list.append(max(10, upper_bound))
        time_lenght_iter = time.time()-t_init_iter
        lists_for_graphics.time_per_iteration.append(time_lenght_iter)

        create_graphique(lists_for_graphics)

        list_of_iterations.append(current_iteration)    
        num_it += 1
...
def create_graphique(lists_for_graphics: Graphic_lists):
    ## affichage
    ...
    line.set_xdata(range(len(lists_for_graphics.cout_min_liste)))
    line2.set_xdata(range(len(lists_for_graphics.facture_list)))
    line3.set_xdata(range(len(lists_for_graphics.upper_bound_list)))
    line4.set_xdata(range(len(lists_for_graphics.time_per_iteration)))
    line5.set_xdata(range(len(lists_for_graphics.num_try_list)))
    line6.set_xdata(range(len(lists_for_graphics.step_it_list)))
    line.set_ydata(lists_for_graphics.cout_min_liste)
    line2.set_ydata(lists_for_graphics.facture_list)
    line3.set_ydata(lists_for_graphics.upper_bound_list)
    line4.set_ydata(lists_for_graphics.time_per_iteration)
    line5.set_ydata(lists_for_graphics.num_try_list)
    line6.set_ydata(lists_for_graphics.step_it_list)
    
    for a in ax.flat:
        a.set_xlim(0, len(lists_for_graphics.cout_min_liste))
        
    
    ax[0,0].set_ylim(-3000, 6000)
    ax[0,1].set_ylim(min(lists_for_graphics.facture_list)-5, max(lists_for_graphics.facture_list)+5)
    ax[1,0].set_ylim(0, max(lists_for_graphics.upper_bound_list))
    ax[1,1].set_ylim(0, max(lists_for_graphics.time_per_iteration))
    ax[2,0].set_ylim(0, max(lists_for_graphics.num_try_list)+1)
    ax[2,1].set_ylim(0, max(lists_for_graphics.step_it_list))
